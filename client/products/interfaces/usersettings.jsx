...
const Usersettings = () => {
  ...
  function changepassword(){
    ...
    let token = localStorage.getItem("userToken");
      ...
      try{
    let response=axios.post(`${import.meta.env.VITE_BACKEND_URL}/users/changepassword`,obj,{
        headers:{
...
showToast("error", err?.response?.data?.message || "Failed to update password");
  ...
  function changecred(){
    let obj={
      ...
      try{
    let response=axios.post(`${import.meta.env.VITE_BACKEND_URL}/users/changecred`,obj,{
        headers:{
Authorization: `Bearer ${token}`
        }
    })
      ...
      <div className='h-screen w-screen flex'>
        ...
        {toast && (
          <div className='fixed top-6 right-6 z-50 animate-[fadeIn_0.2s_ease-out]'>
            <div
              className={`flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg border text-sm font-medium min-w-[260px] ${
                toast.type === "success"
